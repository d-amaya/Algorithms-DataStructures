...
public class NucleoBases {
	...
	public static String find_genes(String dna) {
		...
		for (int i = 0; i < nucleos.size(); i++) {
			...
			int j = i + 1;
			for (; j < nucleos.size() && set.size() < 3; j++) set.add(nucleos.get(j).nucleo);
			j -= 1;
		...
		String answer = "";
		if (start >= 0) answer = dna.substring(start, end + 1);
	...
	private static void validateNucleo(StringBuilder sb, List<Nucleo> nucleos, int start) {
		...
		if (nucleo.equals("ACT") || nucleo.equals("CGT") || nucleo.equals("AGT")) nucleos.add(new Nucleo(start, nucleo));
	}
